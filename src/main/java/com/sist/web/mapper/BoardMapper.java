...
public interface BoardMapper {
	...
	@Select("SELECT id, title, content, time, hit, mem_id, region, category, "
			+ "(SELECT COUNT(*) FROM board_like WHERE board_id = board.id) as like_cnt, "
			+ "(SELECT COUNT(*) FROM board_like WHERE board_id = board.id AND mem_id = #{username}) as isLiked "
	...
	// 2. 좋아요 체크 (현재 이 유저가 좋아요를 눌렀는지 확인용)
	@Select("SELECT COUNT(*) FROM board_like WHERE board_id = #{board_id} AND mem_id = #{mem_id}")
	int boardLikeCheck(@Param("board_id") int board_id, @Param("mem_id") String mem_id);

	// 3. 좋아요 추가
	@Insert("INSERT INTO board_like(like_id, board_id, mem_id) "
			+ "VALUES(board_like_seq.nextval, #{board_id}, #{mem_id})")
	void boardLikeInsert(@Param("board_id") int board_id, @Param("mem_id") String mem_id);

	// 4. 좋아요 취소
	@Delete("DELETE FROM board_like WHERE board_id = #{board_id} AND mem_id = #{mem_id}")
	void boardLikeDelete(@Param("board_id") int board_id, @Param("mem_id") String mem_id);
}