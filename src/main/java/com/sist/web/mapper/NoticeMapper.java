...
public interface NoticeMapper {
	...
	@Select("<script>" +
			...
			"</script>")
	List<NoticeVO> noticeListData(@Param("start") int start, @Param("fd") String fd);
	...
	@Select("SELECT not_id, not_title " +
			"FROM (SELECT not_id, not_title FROM notice " +
			"WHERE not_id < #{not_id} ORDER BY not_id DESC) " +
			"WHERE ROWNUM = 1")
	...
	@Update("UPDATE notice SET not_title = #{not_title}, not_content = #{not_content} " +
			"WHERE not_id = #{not_id}")
