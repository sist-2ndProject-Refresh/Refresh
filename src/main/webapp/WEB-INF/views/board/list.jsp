					...
					<div
						...
						<div class="d-flex align-items-center">
							<select class="form-select me-3" v-model="store.selectedCategory"
								@change="store.changeCategory()"
								style="width: 80px; border-radius: 500px; font-size: 1.2rem; font-weight: 600; height: 45px; border: 1px solid #ccc;">
							...
							</select> <a href="/board/insert" class="btn btn-dark px-3 py-2 shadow-sm"
				...
				<table class="table table-hover">
					<thead>
						<tr>
							...
							<th width="5%" class="text-center">지역</th>
							<th width="5%" class="text-center">카테고리</th>
					...
					<tbody>
						<tr v-for="vo in store.list" :key="vo.id">
							...
							<td class="text-center"><a
								:href="'http://localhost:8080/board/detail?no=' + vo.id">
									{{vo.title}} </a></td>
							<td class="text-center">{{vo.mem_id}}</td>
							<td class="text-center">{{ vo.time.split('T')[0] }}</td>
