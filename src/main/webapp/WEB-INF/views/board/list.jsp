				...
				<div class="row">
					<div class="col-lg-12 text-left">
						...
						<div class="category-container">
							<button v-for="r in ['전체', '서울', '경기/인천', '부산/경상', '대전/충청', '기타 지역']"
								:key="r" class="category-item"
								:class="{ active: store.selectedRegion === r }"
								@click="store.selectedRegion = r; store.curpage = 1; store.dataRecv()">{{ r }}</button>
						</div>
						...
						<div class="breadcrumb__option d-flex justify-content-between align-items-center"
							...
							<div class="d-flex align-items-center">
								...
								<a v-if="store.sessionId && store.sessionId !== 'null'"
			...
			<div class="container">
				<div class="row">
					<table class="table table-hover" style="table-layout: fixed; width: 100%;">
						...
						<tbody>
							<template v-if="store.filteredList && store.filteredList.length > 0">
								<tr v-for="vo in store.filteredList" :key="vo.id">
									...
									<td class="text-center text-muted">{{ vo.time ? vo.time.split('T')[0] : '' }}</td>
									<td class="text-center">{{vo.hit}}</td>
								</tr>
							</template>

							<tr v-if="!store.filteredList || store.filteredList.length === 0">
								<td colspan="7" class="text-center py-5" style="color: #999; font-size: 1.5rem;">
									<i class="fa fa-info-circle"></i> 선택하신 조건에 맞는 게시글이 없습니다.
								</td>
							</tr>

							<tr>
								<td colspan="7" class="text-center pt-5 pagination-wrapper" style="border: none;">
									<span @click="store.prev()" style="cursor: pointer; margin-right: 15px;" v-if="store.curpage > 1"> [이전] </span> 
									
									<span v-for="page in store.totalpage" :key="page"
										@click="store.curpage = page; store.dataRecv()"
										:class="{ 'fw-bold text-dark border-bottom border-dark': store.curpage === page }"
										style="cursor: pointer; margin: 0 12px; padding-bottom: 2px;"> {{ page }} </span> 
										
									<span @click="store.next()" style="cursor: pointer; margin-left: 15px;" v-if="store.curpage < store.totalpage"> [다음] </span>
								</td>
							</tr>
						</tbody>
					</table>
