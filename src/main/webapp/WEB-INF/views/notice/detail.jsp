...
<body>
	<section class="py-5" id="notice_detail">
		<div class="container" style="max-width: 1500px;"
			...
			<div class="article-content mb-5">{{ store.vo.not_content }}</div>

			<div class="border-top border-bottom py-4 mb-5">
				<div class="d-flex align-items-center mb-3">
					<div class="nav-label">이전글 ▲</div>
					<div v-if="store.prevVo?.not_id">
						<a :href="'/notice/detail?no=' + store.prevVo.not_id"
							class="nav-link"> {{ store.prevVo.not_title }} </a>
					</div>
					<div v-else class="text-muted" style="font-size: 1.2rem;">이전
						글이 없습니다.</div>
				</div>
				...
				<div class="d-flex align-items-center">
					...
					<div v-if="store.nextVo?.not_id">
						<a :href="'/notice/detail?no=' + store.nextVo.not_id"
							class="nav-link"> {{ store.nextVo.not_title }} </a>
					</div>
					<div v-else class="text-muted" style="font-size: 1.2rem;">다음
						글이 없습니다.</div>
				</div>
			</div>
