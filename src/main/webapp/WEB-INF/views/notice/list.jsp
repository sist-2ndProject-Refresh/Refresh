	...
	<section class="breadcrumb-section set-bg"
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-12">
					<h1
						style="font-size: 3.5rem; font-weight: 800; color: #111; letter-spacing: -0.03em; margin-bottom: 15px;">공지사항</h1>
		...
		<div class="container">
					...
					<table class="table table-hover notice-table">
						...
						<tbody>
							<tr v-for="vo in store.list" :key="vo.not_id"
								...
								<td>{{vo.not_id}}</td>
								<td class="text-left px-3"><a
									:href="'/notice/detail?no=' + vo.not_id"
									class="text-decoration-none text-dark"> {{vo.not_title}} </a></td>
								<td>{{vo.mem_id}}</td>
								<td>{{vo.not_date}}</td>
								<td>{{vo.view_cnt}}</td>
							</tr>
							<tr v-if="store.list.length === 0">
								<td colspan="5" class="py-5 text-center text-muted">등록된
									공지사항이 없습니다.</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="row mt-4" v-if="store.totalpage > 0">
				<div class="col-12 text-center">
					<span @click="store.prev()" style="cursor: pointer"
						class="page-link-custom"> [이전] </span> <span
						v-for="page in Array.from({length: store.endPage - store.startPage + 1}, (_, i) => store.startPage + i)"
						:key="page" @click="store.curpage = page; store.dataRecv()"
						:class="{ 'active': store.curpage === page }"
						class="page-link-custom"> {{ page }} </span> <span
						@click="store.next()" style="cursor: pointer"
						class="page-link-custom"> [다음] </span>
				</div>
			</div>
		</div>
	</section>
