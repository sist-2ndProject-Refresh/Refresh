			    	...
			    	<div class="d-flex align-items-stretch">
				        <div class="category-scroll-box px-3 py-2 border rounded" style="max-height: 200px; overflow-y: auto; width: 200px; max-width: 500px;">
				            <label class="d-flex align-items-center mb-2" v-for="cate1 in store.detailData.cate1List" :key="cate1" style="cursor: pointer;">
				                <input type="radio" v-model="store.detailData.category1" ref="categoryRef_1" :value="cate1.first_id" @change="store.loadCategorySecond(cate1.first_id)" class="form-check-input me-2" >
						...
						<div v-if="store.detailData.cate2List.length > 0">
					        <div class="category-scroll-box px-3 py-2 border rounded" style="max-height: 200px; overflow-y: auto; width: 200px; max-width: 500px;">
					            <label class="d-flex align-items-center mb-2" v-for="cate2 in store.detailData.cate2List" :key="cate2" style="cursor: pointer;">
					                <input type="radio" v-model="store.detailData.category2" ref="categoryRef_2" :value="cate2.second_id" @change="store.loadCategoryThird(cate2.second_id)" class="form-check-input me-2" >
				...
				<div class="mar-top-50 d-flex align-items-stretch gap-100">
					...
					<div>
						<div class="d-flex gap-100" ref="deliveryRef">
					        <label class="d-flex align-items-center" style="cursor: pointer;">
					            <input type="radio" v-model="store.detailData.includeDelivery" name="delivery-price" :value="0" class="form-check-input me-2" checked>
						...
						<div v-if="store.detailData.includeDelivery === 1">
							<div class="d-flex mar-top-50 gap-100" ref="csvRef"> <!-- 택배비 별도일 때 보이도록 하기 display:none  -->
								<label class="d-flex align-items-center" style="cursor: pointer;">
						            <input type="checkbox" v-model="store.detailData.isGS" class="form-check-input me-2" style="width: 20px; height: 20px;">
						            <span class="fs-2 fw-normal">GS반값택배</span>
						        </label>
								<label class="d-flex align-items-center" style="cursor: pointer;">
						            <input type="checkbox" v-model="store.detailData.isCU" class="form-check-input me-2" style="width: 20px; height: 20px;">
							...
							<div class="d-flex align-items-center gap-2" style="margin-top: 20px;">	
								...
								<input class="form-control fs-2 text-right" v-model="store.detailData.cvsPrice" type="number" min="1" style="width: 200px; height: 40px;" placeholder="금액을 작성해주세요" >
								<span class="fs-2">원</span>
							</div>
						</div>
					</div>
			...
			<div>
				...
				<div class="mar-top-50 d-flex align-items-stretch gap-100">
					...
					<label class="d-flex align-items-stretch" style="cursor: pointer;">
			            <input type="checkbox" v-model="store.detailData.isDirect" ref="directRef" class="form-check-input me-2" style="width: 17px; height: 17px;">
			            <span class="fs-3 fw-normal">직거래 여부</span>
			        </label>
					<div style="margin-bottom:50px" ref="addressRef" v-if="store.detailData.isDirect">	
