...
<body>
		...
		<div class="row mx-0 align-items-center" style="width: 100%;">
		    ...
		    <div class="text-right" style="margin-top: 20px;">
		    	<sec:authorize access="hasRole('USER')">
		    		<a href="/rental/new" class="btn btn-dark">등록하기</a>
		    	</sec:authorize>
		    </div>
		</div>
		<!-- 상품 출력 -->
					...
					<div class="product-card position-relative">
						<a :href="'/rental/detail?no=' + vo.no">
							...
							<div class="card-detail mt-3">
								...
								<div class="card-price fw-bold fs-2 text-right" style="margin-top: 10px">{{ Number(vo.price).toLocaleString() }}원 / {{vo.days}}일</div>
							</div>
							<hr class="hr-st">
	...
	<script src="/vue/rental/rentalSelectStore.js"></script>
	<script>
		const {createApp, onMounted} = Vue
		const {createPinia} = Pinia
		
		const rentalApp = createApp({
			setup(){
		...
		rentalApp.use(createPinia())
		rentalApp.mount('#rental_list')
	</script>
</body>
</html>